...
export default function ModalPage() {
  ...
  useEffect(() => {
    if (Index !== "") {
      const { modelName, _id } = data[Index];
  ...
  const getModel = async () => {
    const response = await getApihandler("/getModels/All");
    // console.log("reponse----->", response);
    if (response.status === 200) {
      setData(response.data);
    }
  };
  const addModel = async (value) => {
    const response =
      Index === ""
        ? await postApihandler("/addModels", value)
        : await putApihandler(`/updateModels/${Id}`, value);
  ...
  const deleteData = async (index) => {
    ...
    // console.log("id=>", _id);
    const deleteres = await deleteApiihandler("/deleteModels/", _id);
      ...
      >
        <Box sx={style}>
          ...
          >
            ...
            <Box
              component="form"
              onSubmit={handleSubmit(addModel)}
              noValidate
                  ...
                  .map((val, index) => {
                    return (
                      <TableRow hover role="checkbox" tabIndex={-1} key={index}>
                        ...
                        <TableCell>{val.modelName}</TableCell>
