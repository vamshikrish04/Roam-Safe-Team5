...
export default function Mechanic() {
  ...
  const verifyMech = async () => {
    ...
    const reponse = putApihandler(`/verifyMechanicByAdmin/${mechId}`, aaray);
    console.log("------->", reponse);
    setMechId("");
    setFlage();
    getMachanic();
  };
  const getMachanic = async () => {
    const response = await getApihandler("/getAllMechanic/All");
                  ...
                  .map((val, index) => {
                    return (
                      <TableRow hover role="checkbox" tabIndex={-1} key={index}>
                        ...
                        <TableCell>
                          <Link to={`/mechanics-detail/${val._id}`}>
