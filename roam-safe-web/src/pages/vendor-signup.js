...
export default function VendorSignUp() {
  ...
  const onSubmit = async (value) => {
    ...
    const response = await postApihandler("/mechanicSignUP", formData);
    console.log("response===>", response);

    if (response.status === 200) {
      const service = {
        service_provided: personName,
      };
      console.log("Id-->", response.data._id);
      const res = await putApihandler(
        `/addServiesProvided/${response.data._id}`,
        service
      );
      console.log("addServiesProvided=========>", res);
      Swal.fire({
            ...
            >
              ...
              <Box>
                <form onSubmit={handleSubmit(onSubmit)}>
                  ...
                  />
                  <FormControl className={styles.service_input_size}>
                    <InputLabel variant="standard" color="error">
                      Services
                    </InputLabel>
                    <Select
                      required
