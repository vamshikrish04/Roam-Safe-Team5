...
export default function BookingHistoryRagular() {
  ...
  const VerifyBooking = async () => {
    ...
    const res = await putApihandler(
      `/bookingAcceptOrRejectByMachenic/${BookingId}`,
      value
    );
  ...
  const getBookings = async () => {
    console.log("api url - ", `/getMechanicBooking/mechanicId_${userId}`);
    const response = await getApihandler(
      `/getMechanicBooking/mechanicId_${userId}`
    );

    console.log("reponse----->143", response);
  ...
  const onSubmit = async (value) => {
    ...
    const res = await putApihandler(
      `/addAmountForRegularService/${BookingId}`,
      item
    );
    if (res.status === 200) {
      Swal.fire({
